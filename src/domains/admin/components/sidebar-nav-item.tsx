import { useEffect, useState } from 'react';
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from '@/components/ui/collapsible';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuLabel,
  DropdownMenuSeparator,
  DropdownMenuTrigger
} from '@/components/ui/dropdown-menu';
import { Tooltip, TooltipContent, TooltipTrigger } from '@/components/ui/tooltip';
import { cn } from '@/lib/utils';
import { ICON_MAP, type SidebarItem } from '../data';
import Link from 'next/link';
import { IconChevronDown } from '@tabler/icons-react';

export function SidebarNavItem({
  item,
  isCollapsed,
  pathname
}: {
  item: SidebarItem;
  isCollapsed: boolean;
  pathname: string;
}) {
  const hasChildren = !!(item.children && item.children.length > 0);
  const isChildActive =
    hasChildren && item.children?.some((c) => pathname.startsWith(c.href || ''));
  const [isOpen, setIsOpen] = useState(!!isChildActive);

  useEffect(() => {
    if (isChildActive) setIsOpen(true);
  }, [isChildActive]);

  const isActive = pathname === item.href || isChildActive;

  const itemClasses = cn(
    'group relative flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium transition-all duration-200 outline-none mb-1',
    isActive
      ? 'bg-secondary/80 text-foreground shadow-[inset_0_1px_0_0_rgba(255,255,255,0.05)] border border-border/50'
      : 'text-muted-foreground hover:bg-accent/40 hover:text-foreground border border-transparent',
    isCollapsed && 'justify-center p-0 h-10 w-10 mx-auto rounded-xl'
  );

  const renderIcon = (iconName?: string, active?: boolean) => {
    // @ts-ignore
    const IconComponent = iconName ? ICON_MAP[iconName] : null;
    return (
      <div
        className={cn(
          'flex shrink-0 items-center justify-center transition-colors duration-200',
          active ? 'text-primary' : 'text-muted-foreground group-hover:text-foreground'
        )}
      >
        {IconComponent ? (
          <IconComponent size={20} strokeWidth={active ? 2.5 : 2} />
        ) : (
          <div
            className={cn(
              'h-1.5 w-1.5 rounded-full bg-current transition-all',
              active ? 'scale-125' : 'opacity-30'
            )}
          />
        )}
      </div>
    );
  };

  // --- CASE 1: COLLAPSED WITH CHILDREN (Opens Menu on Click) ---
  if (isCollapsed && hasChildren) {
    return (
      <DropdownMenu>
        <Tooltip delayDuration={0}>
          <TooltipTrigger asChild>
            <DropdownMenuTrigger asChild>
              <button className={cn(itemClasses, 'cursor-pointer overflow-visible outline-none')}>
                {renderIcon(item.icon, isActive)}
                {isChildActive && (
                  <div className='bg-primary border-background absolute top-1 right-1 h-2 w-2 rounded-full border-2' />
                )}
              </button>
            </DropdownMenuTrigger>
          </TooltipTrigger>
          {/* Standard Tooltip shows only the Title on Hover */}
          <TooltipContent side='right' sideOffset={12} className='z-50 text-xs font-bold'>
            {item.label}
          </TooltipContent>
        </Tooltip>

        {/* Dropdown Content Portals automatically to Body in Shadcn */}
        <DropdownMenuContent
          side='right'
          sideOffset={15}
          className='bg-popover/95 border-border/50 z-100 w-56 rounded-xl p-2 shadow-2xl backdrop-blur-md'
        >
          <DropdownMenuLabel className='text-muted-foreground/50 px-3 py-2 text-[10px] font-bold tracking-widest uppercase'>
            {item.label}
          </DropdownMenuLabel>
          <DropdownMenuSeparator className='opacity-50' />
          <div className='mt-1 space-y-0.5'>
            {item.children?.map((child) => (
              <DropdownMenuItem key={child.href} asChild>
                <Link
                  href={child.href ?? '#'}
                  className={cn(
                    'flex w-full cursor-pointer items-center justify-between rounded-lg px-3 py-2 text-xs font-medium transition-colors outline-none',
                    pathname === child.href
                      ? 'bg-primary/10 text-primary'
                      : 'text-muted-foreground hover:text-foreground hover:bg-accent'
                  )}
                >
                  {child.label}
                  {pathname === child.href && <div className='bg-primary h-1 w-1 rounded-full' />}
                </Link>
              </DropdownMenuItem>
            ))}
          </div>
        </DropdownMenuContent>
      </DropdownMenu>
    );
  }

  // --- CASE 2: EXPANDED WITH CHILDREN (Collapsible) ---
  if (hasChildren && !isCollapsed) {
    return (
      <Collapsible open={isOpen} onOpenChange={setIsOpen} className='w-full'>
        <CollapsibleTrigger asChild>
          <button className={cn(itemClasses, 'w-full cursor-pointer justify-between')}>
            <div className='flex items-center gap-3'>
              {renderIcon(item.icon, isActive)}
              <span className={cn('transition-colors', isActive ? 'font-semibold' : 'font-medium')}>
                {item.label}
              </span>
            </div>
            <IconChevronDown
              className={cn(
                'h-3.5 w-3.5 opacity-40 transition-transform duration-300',
                isOpen && 'text-primary rotate-180 opacity-100'
              )}
            />
          </button>
        </CollapsibleTrigger>
        <CollapsibleContent className='border-border/60 mt-1 ml-5.5 space-y-0.5 border-l pl-4'>
          {item.children?.map((child) => {
            const isChildLinkActive = pathname === child.href;
            return (
              <Link
                key={child.href}
                href={child.href ?? '#'}
                className={cn(
                  'relative block rounded-lg px-3 py-2 text-xs transition-all',
                  isChildLinkActive
                    ? 'text-foreground bg-accent/50 font-bold shadow-sm'
                    : 'text-muted-foreground hover:text-foreground hover:bg-accent/30'
                )}
              >
                {isChildLinkActive && (
                  <div className='bg-primary absolute top-1/2 -left-4.25 h-1.5 w-1.5 -translate-y-1/2 rounded-full' />
                )}
                {child.label}
              </Link>
            );
          })}
        </CollapsibleContent>
      </Collapsible>
    );
  }

  // --- CASE 3: SINGLE ITEM (No Children) ---
  return (
    <Tooltip delayDuration={0}>
      <TooltipTrigger asChild>
        <Link href={item.href || '#'} className={cn(itemClasses)}>
          {renderIcon(item.icon, isActive)}
          {!isCollapsed && <span className='flex-1 truncate'>{item.label}</span>}
          {isActive && isCollapsed && (
            <div className='bg-primary absolute top-1/2 left-0 h-5 w-1 -translate-y-1/2 rounded-r-full' />
          )}
        </Link>
      </TooltipTrigger>
      {isCollapsed && (
        <TooltipContent
          side='right'
          sideOffset={10}
          className='bg-popover border-border z-50 px-3 py-1.5 text-xs font-bold shadow-xl'
        >
          {item.label}
        </TooltipContent>
      )}
    </Tooltip>
  );
}
